# Google News Latest — Agent Workspace
...
`/root/Hermes/projects/sd-google-news-latest-backend-mvp-v2026.06.30.1` — mounted at the **same absolute path** on the host and inside the container. **Do all
...
- **Never relocate work to `/tmp`, bare `/root`, `~`, or any non-mounted path.** Those are container-local
  ...
  `/root/Hermes/projects/sd-google-news-latest-backend-mvp-v2026.06.30.1` persist.
...
workspace (`--workspace dir:/root/Hermes/projects/sd-google-news-latest-backend-mvp-v2026.06.30.1`). The verifier **gates** every milestone: it passes only on
