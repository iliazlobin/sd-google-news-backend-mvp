...
from verify.acceptance.conftest import assert_json_200, assert_404, assert_422
...
def test_feed_respects_limit(client, known_user):
    ...
    user_id = known_user["user_id"]
    feed = assert_json_200(
        client.get(f"/v1/feed?user_id={user_id}&limit=2")
    )
...
def test_feed_unknown_user_404(client):
    ...
    assert_404(
        client.get("/v1/feed?user_id=00000000-0000-0000-0000-00000000ffff")
    )
...
def test_feed_freshness_decay_visible(client, seed_articles, known_user):
    ...
    user_id = known_user["user_id"]
    feed = assert_json_200(
        client.get(f"/v1/feed?user_id={user_id}&limit=30")
    )
