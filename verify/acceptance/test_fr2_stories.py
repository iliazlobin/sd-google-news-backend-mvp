...
from verify.acceptance.conftest import assert_404, assert_json_200
...
def test_story_detail_unknown_404(client):
    ...
    assert_404(client.get("/v1/stories/00000000-0000-0000-0000-00000000dead"))
...
def test_story_articles_paginated(client, seed_articles):
    ...
    page1 = assert_json_200(client.get(f"/v1/stories/{story_id}/articles?limit=1&offset=0"))
    ...
    if page1.get("total", 0) > 1:
        page2 = assert_json_200(client.get(f"/v1/stories/{story_id}/articles?limit=1&offset=1"))
        assert len(page2["articles"]) == 1
        # Should be different article
        if page1["articles"] and page2["articles"]:
            assert page1["articles"][0]["article_id"] != page2["articles"][0]["article_id"]


def test_story_articles_unknown_story_404(client):
    """Articles sub-resource on unknown story returns 404."""
    assert_404(client.get("/v1/stories/00000000-0000-0000-0000-00000000dead/articles"))
